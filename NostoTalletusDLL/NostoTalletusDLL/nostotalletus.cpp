    ...
    ui(new Ui::NostoTalletus)
{
    ...
    QString tili_url=getBaseURLTili(), asiakasTili_url = getBaseURLpvNostoRaja();
    QNetworkRequest requestTili((tili_url)), requestAsiakasTili((asiakasTili_url));

    //WEBTOKEN ALKU
    QByteArray myToken="";
    requestTili.setRawHeader(QByteArray("Authorization"),(myToken));
    requestAsiakasTili.setRawHeader(QByteArray("Authorization"),(myToken));
    //WEBTOKEN LOPPU
...
{
    return "http://localhost:3000/tili/9";
}

QString NostoTalletus::getBaseURLpvNostoRaja() // Poista myöhemmin ?
{
    return "http://localhost:3000/asiakasTili/3/9";
}
